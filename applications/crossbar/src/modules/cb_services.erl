...
post(Context, ?TOPUP) ->
    ...
    case kz_services_topup:top_up(cb_context:account_id(Context), Amount) of
...
is_allowed(Context) ->
    ResellerId = kz_services_reseller:find_id(
...
get_topup_amount(Context) ->
    ReqData = cb_context:req_data(Context),
    wht_util:dollars_to_units(
      kz_json:get_integer_value(<<"amount">>, ReqData, 0)
     ).
